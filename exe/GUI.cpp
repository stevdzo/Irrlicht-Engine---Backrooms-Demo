...
void GUI::show_collected_papers() {
	...
	for (size_t i = 1; i <= PAPER_COUNT; i++) {
		...
		ss_path << i;
		std::string name = "paper_" + ss_path.str();
		std::string path = "resources/gui/papers/" + name + ".png";
		

		irr::video::ITexture* paper_texture = device->getVideoDriver()->getTexture(path.c_str());
		irr::gui::IGUIImage* paper_image = device->getGUIEnvironment()->addImage(paper_texture, irr::core::position2di(20, SCREEN_HEIGHT / 2 + offset_y * i));
		paper_image->setName(name.c_str());
		paper_image->setVisible(false);

		for (auto& paper : collected_papers_id) {	
			if (paper == paper_image->getName()) {		
				paper_image->setVisible(true);
			}
		}
		collected_papers.push_back(paper_image);
	}*/
}
